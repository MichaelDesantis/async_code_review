...
* And then the best explanation of promises ever =  https://github.com/mattdesl/promise-cookbook

* MOAR PROMISES! https://scotch.io/tutorials/javascript-promises-for-dummies

### 08. Promise 'gotchas' and promise API in more depth.

* Before we go too far, I should mention that the JavaScript try / catch mechanism cannot be used to intercept errors generated by asynchronous APIs. A common mistake for beginners is to try to use throw inside a Node.js style callback:
https://nodejs.org/dist/latest/docs/api/errors.html#errors_node_js_error_codes 

* ES6 Promises cannot be canceled, our best quick workaround at this time is promise.race(). There are however, other promise libraries that support this behavior. And there are proposals on how to fix this coming with newer versions of JS/ECMAScript. Although for this reason, some people opt for observables over promises. 

* Also worth noting, a promise resolves to a single value asynchronously. An observable can resolve or emit multiple values asynchronously over time. Observables however, are beyond the scope of this lesson.
